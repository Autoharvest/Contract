...
contract Storage {
  ...
  address public governance;  //0xf00dd244228f51547f0563e60bca65a30fbf5f7f 
  address public controller;  //0x222412af183bceadefd72e4cb1b71f1889953b1c 
...
// Naming was not adjusted for easy diff0xf00dd244228f51547f0563e60bca65a30fbf5f7f 
contract CRVStrategySwerve is IStrategy, ProfitNotifier {
  ...
  address public gauge;   ///0xb4d0c929cd3a1fbdc6d57e7d3315cf0c4d6b4bfa 

  // the reward minter
  address public mintr;  ////0x2c988c3974ad7e604e276ae0294a7228def67974

  // the address for the CRV token
  address public crv;  ///0xb8baa0e4287890a5f79863ab62b7f175cecbd433
...
////赎回一定数量的mix代币并将mix代币转换为wbtc并转移给vault合约
  /**
...
////赎回所有的mix代币并将mix代币转换为wbtc并转移给vault合约
  /**
  ...
  */
  function withdrawAllToVault() external restricted {
    ...
    mixToWBTC(uint256(~0)); //将mix代币转换成wbtc代币
    // we can transfer the asset to the vault
    uint256 actualBalance = IERC20(wbtc).balanceOf(address(this));
    if (actualBalance > 0) {
  ...
  */
  function investAllUnderlying() internal {
    ...
    if (mixTokenBalance > 0) {
      ...
      Gauge(gauge).deposit(mixTokenBalance);///抵押所有的mix代币
    }
  }
  ...
  */
  function doHardWork() public restricted {
    claimAndLiquidateCrv();// 收获crv奖励，并将奖励转换成wbtc
    investAllUnderlying(); //将所有的btc都转换成mix代币并抵押
...
///////////////// 查询接口，用于查询wbtc的余额，数额包含了mixed可以兑换成的wbtc的数量
  /**
...
///////////////// 收获crv奖励，并将奖励转换成webt
  ...
  */
  function claimAndLiquidateCrv() internal {
    ...
    Mintr(mintr).mint(gauge);   // 这一行是将crv这个token增发到本合约中，具体的数额通过gauge合约中的integrate_fraction方法计算
    ...
    if (crvBalance > sellFloor) {
      ...
      //上面是进行授权等操作，下面这一行是将crv这个代币兑换成wbtc
